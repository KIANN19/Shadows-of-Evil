...
var enemy_2 = preload("res://Scenes/Boss.tscn")
onready var player = $Player
#anropar enemy och spelaren

func _ready():
	$Enemy_spawn_timer.wait_time = Global.wait_time_enemies
	$Boss_spawn_timer.wait_time = Global.wait_time_bosses
	randomize()
...
func _on_Boss_spawn_timer_timeout():
	var boss = enemy_2.instance()
	var section = randi() % 4 
	if section == 0:
		boss.global_position = player.global_position + Vector2(-WIDTH/2 - 20, rand_range(-HEIGHT/2, HEIGHT/2))
	elif section == 1:
		boss.global_position = player.global_position + Vector2(WIDTH/2 + 20, rand_range(-HEIGHT/2, HEIGHT/2))
	elif section == 2:
		boss.global_position = player.global_position + Vector2(rand_range(-WIDTH/2, WIDTH/2), -HEIGHT/2 - 20)
	else:
		boss.global_position = player.global_position + Vector2(rand_range(-WIDTH/2, WIDTH/2), HEIGHT/2 + 20)
	$Navigation2D/NavigationPolygonInstance.add_child(boss)
